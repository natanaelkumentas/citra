...
class CameraChoiceDialog(tk.Toplevel):
    def __init__(self, parent, mode='save', callback=None):
        ...
        # ── Divider ──
        divider = tk.Frame(main, bg=self.colors["border"], height=1)
        ...
        cancel_btn = tk.Button(
            btn_frame, text="Cancel", width=14,
        ...
        # Open button (right)
        open_btn = tk.Button(
            btn_frame, text="✅ Open", width=14,
            command=self.on_open,
            font=("Segoe UI", 10, "bold"),
            bg=self.colors["accent_blue"], fg="white",
            activebackground="#2488C2",
            activeforeground="white",
            bd=1, relief="raised", cursor="hand2"
        )
        open_btn.pack(side="right", padx=(0, 0))
