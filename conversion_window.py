...
from utils_image import detect_image_type, format_file_size, estimate_image_bytes
...
class ConversionWindow(tk.Toplevel):
    ...
    def start_camera(self):
        # start camera only if no file currently selected
        try:
            self.camera = cv2.VideoCapture(0 if self.use_internal else self.camera_url)
            if not self.camera.isOpened():
                self.live_label.config(text="Gagal membuka kamera")
                return
    ...
    def convert(self):
        ...
        if self.conv_mode == "rgb_to_gray":
            ...
            # threshold to binary
            _, binary = cv2.threshold(self.gray_frame, self.threshold.get(), 255, cv2.THRESH_BINARY)
            if self.inverted:
    ...
    def on_threshold(self, v):
        if self.gray_frame is None:
            return
        _, binary = cv2.threshold(self.gray_frame, int(v), 255, cv2.THRESH_BINARY)
        if self.inverted:
            binary = cv2.bitwise_not(binary)
        self.converted_frame = binary
    ...
    def stop_camera_for_file(self):
        # disable camera updating and release device so selected file shows
        self.is_running = False
        if self.camera:
            try:
                self.camera.release()
    ...
    def resume_live(self):
        # resume the live camera (if camera available)
        # clear selected file marker
        ...
        try:
            self.camera = cv2.VideoCapture(0 if self.use_internal else self.camera_url)
            if not self.camera.isOpened():
                messagebox.showwarning("Warning", "Tidak bisa membuka kamera.")
                return
            self.is_running = True
            self.update_camera()
